...
describe("computeDioramaPanMetersFromStick", () => {
    ...
    it("footprintHalfSizeMが下限未満でも下限値で速度を計算する（低ズームで操作不能にならない）", () => {
        const result = computeDioramaPanMetersFromStick(
            { x: 1, y: 0 },
            1,
            5, // 下限(既定20)未満
            0.6,
            { minFootprintHalfSizeForSpeedM: 20 },
        );
        expect(result.eastM).toBeCloseTo(0.6 * 20, 6);
    });
});

describe("computeFootprintHalfSizeFactorFromStick", () => {
    it("入力がdeadzone以下、またはdt/rateが不正なら係数1（変化なし）", () => {
        expect(computeFootprintHalfSizeFactorFromStick(0.05, 1, 2)).toBe(1);
        expect(computeFootprintHalfSizeFactorFromStick(1, 0, 2)).toBe(1);
        expect(computeFootprintHalfSizeFactorFromStick(1, 1, 0)).toBe(1);
        expect(computeFootprintHalfSizeFactorFromStick(1, 1, -1)).toBe(1);
    });

    it("前方向(y<0)入力で係数が1未満（ズームイン=半径縮小）になる", () => {
        const factor = computeFootprintHalfSizeFactorFromStick(-1, 1, 2);
...
describe("clampFootprintHalfSizeM", () => {
    it("範囲内の値はそのまま返す", () => {
        expect(clampFootprintHalfSizeM(800)).toBe(800);
    });
    ...
    it("カスタムのmin/maxを指定できる", () => {
        expect(clampFootprintHalfSizeM(50, 100, 200)).toBe(100);
        expect(clampFootprintHalfSizeM(300, 100, 200)).toBe(200);
    });
});

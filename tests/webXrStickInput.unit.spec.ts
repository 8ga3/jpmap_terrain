/**
 * `src/lib/webxr/webXrStickInput.ts` のunit test。
 *
 * @remarks
 * `src/demos/diorama/dioramaControllerMapping.ts` の移植元テスト
 * （`tests/dioramaControllerMapping.unit.spec.ts`）と同等の観点を、
 * 公開APIとしてのリネーム後シグネチャに合わせて検証する。
 */
...
describe("computePanMetersFromStick", () => {
    it("入力が全てdeadzone以下なら移動量0", () => {
        const result = computePanMetersFromStick({ x: 0.05, y: 0.05 }, 1, 800, 0.6);
        expect(result.eastM).toBe(0);
        expect(result.northM).toBe(0);
    });

    it("dtSecondsが0以下なら移動量0", () => {
        expect(computePanMetersFromStick({ x: 1, y: 1 }, 0, 800, 0.6)).toEqual({ eastM: 0, northM: 0 });
        expect(computePanMetersFromStick({ x: 1, y: 1 }, -1, 800, 0.6)).toEqual({ eastM: 0, northM: 0 });
    });

    it("viewScaleMに比例した速度でx=東、-y=北方向へ移動する", () => {
        // deadzone適用後 x=1, y=1 → speed = 0.6 * 800 = 480 m/s
        const result = computePanMetersFromStick({ x: 1, y: 1 }, 1, 800, 0.6);
        expect(result.eastM).toBeCloseTo(480, 6);
        // y軸は前方向が負値の規約のため、+y入力は南（northMは負）になる。
        expect(result.northM).toBeCloseTo(-480, 6);
    });

    it("viewScaleMが下限未満でも下限値で速度を計算する（低ズームで操作不能にならない）", () => {
        const result = computePanMetersFromStick(
            { x: 1, y: 0 },
            1,
            5, // 下限(既定20)未満
            0.6,
            { minViewScaleForSpeedM: 20 },
        );
        expect(result.eastM).toBeCloseTo(0.6 * 20, 6);
    });
});

describe("computeZoomFactorFromStick", () => {
    it("入力がdeadzone以下、またはdt/rateが不正なら係数1（変化なし）", () => {
        expect(computeZoomFactorFromStick(0.05, 1, 2)).toBe(1);
        expect(computeZoomFactorFromStick(1, 0, 2)).toBe(1);
        expect(computeZoomFactorFromStick(1, 1, 0)).toBe(1);
        expect(computeZoomFactorFromStick(1, 1, -1)).toBe(1);
    });

    it("前方向(y<0)入力で係数が1未満（ズームイン=スケール縮小）になる", () => {
        const factor = computeZoomFactorFromStick(-1, 1, 2);
        expect(factor).toBeLessThan(1);
        expect(factor).toBeCloseTo(0.5, 10); // 2^(-1*1) = 0.5
    });

    it("後方向(y>0)入力で係数が1超過（ズームアウト=スケール拡大）になる", () => {
        const factor = computeZoomFactorFromStick(1, 1, 2);
...
describe("clampViewScaleM", () => {
    it("範囲内の値はそのまま返す", () => {
        expect(clampViewScaleM(800)).toBe(800);
    });

    it("下限未満は下限へ、上限超過は上限へクランプする", () => {
        expect(clampViewScaleM(DEFAULT_VIEW_SCALE_MIN_M - 50)).toBe(DEFAULT_VIEW_SCALE_MIN_M);
        expect(clampViewScaleM(DEFAULT_VIEW_SCALE_MAX_M + 1000)).toBe(DEFAULT_VIEW_SCALE_MAX_M);
    });

    it("NaN/Infinityは下限側へ丸める", () => {
        expect(clampViewScaleM(NaN)).toBe(DEFAULT_VIEW_SCALE_MIN_M);
        expect(clampViewScaleM(Infinity)).toBe(DEFAULT_VIEW_SCALE_MAX_M);
        expect(clampViewScaleM(-Infinity)).toBe(DEFAULT_VIEW_SCALE_MIN_M);
    });

    it("カスタムのmin/maxを指定できる", () => {
        expect(clampViewScaleM(50, 100, 200)).toBe(100);
        expect(clampViewScaleM(300, 100, 200)).toBe(200);
    });
});
...
describe("computeRotationRadFromStick", () => {
    it("入力がdeadzone以下なら回転角0", () => {
        expect(computeRotationRadFromStick(0.05, 1)).toBe(0);
    });

    it("dtSecondsが0以下なら回転角0", () => {
        expect(computeRotationRadFromStick(1, 0)).toBe(0);
        expect(computeRotationRadFromStick(1, -1)).toBe(0);
    });

    it("rotationSpeedRadPerSecが非有限なら回転角0", () => {
        expect(computeRotationRadFromStick(1, 1, NaN)).toBe(0);
        expect(computeRotationRadFromStick(1, 1, Infinity)).toBe(0);
    });

    it("フルスティック入力で speed*dt の回転角を返す", () => {
        const result = computeRotationRadFromStick(1, 1, DEFAULT_ROTATION_SPEED_RAD_PER_SEC);
...
describe("computeHeightMetersFromTriggers", () => {
    it("両トリガー未入力なら高さ変更量0", () => {
        expect(computeHeightMetersFromTriggers(0, 0, 1)).toBe(0);
    });

    it("dtSecondsが0以下なら高さ変更量0", () => {
        expect(computeHeightMetersFromTriggers(0, 1, 0)).toBe(0);
        expect(computeHeightMetersFromTriggers(0, 1, -1)).toBe(0);
    });

    it("heightSpeedMPerSecが非有限なら高さ変更量0", () => {
        expect(computeHeightMetersFromTriggers(0, 1, 1, NaN)).toBe(0);
    });

    it("右トリガーのみフル押下で上昇（正の変更量）", () => {
        const result = computeHeightMetersFromTriggers(0, 1, 1, DEFAULT_HEIGHT_SPEED_M_PER_SEC);
        expect(result).toBeCloseTo(DEFAULT_HEIGHT_SPEED_M_PER_SEC, 10);
    });

    it("左トリガーのみフル押下で下降（負の変更量）", () => {
        const result = computeHeightMetersFromTriggers(1, 0, 1, DEFAULT_HEIGHT_SPEED_M_PER_SEC);
        expect(result).toBeCloseTo(-DEFAULT_HEIGHT_SPEED_M_PER_SEC, 10);
    });

    it("両方フル押下は相殺されて0", () => {
        expect(computeHeightMetersFromTriggers(1, 1, 1, DEFAULT_HEIGHT_SPEED_M_PER_SEC)).toBe(0);
    });

    it("範囲外・非有限のトリガー値は0/1へクランプしてから計算する", () => {
        const result = computeHeightMetersFromTriggers(-1, 1.5, 1, DEFAULT_HEIGHT_SPEED_M_PER_SEC);
        expect(result).toBeCloseTo(DEFAULT_HEIGHT_SPEED_M_PER_SEC, 10);
        expect(computeHeightMetersFromTriggers(NaN, 1, 1, DEFAULT_HEIGHT_SPEED_M_PER_SEC)).toBeCloseTo(
            DEFAULT_HEIGHT_SPEED_M_PER_SEC,
            10,
        );
    });
...
describe("clampHeightOffsetM", () => {
    it("範囲内の値はそのまま返す", () => {
        expect(clampHeightOffsetM(0.1)).toBe(0.1);
    });

    it("下限未満は下限へ、上限超過は上限へクランプする", () => {
        expect(clampHeightOffsetM(DEFAULT_HEIGHT_OFFSET_MIN_M - 1)).toBe(DEFAULT_HEIGHT_OFFSET_MIN_M);
        expect(clampHeightOffsetM(DEFAULT_HEIGHT_OFFSET_MAX_M + 1)).toBe(DEFAULT_HEIGHT_OFFSET_MAX_M);
    });

    it("NaNは0（オフセット無し）側へフォールバックしてからクランプする", () => {
        expect(clampHeightOffsetM(NaN)).toBe(0);
    });

    it("カスタムのmin/maxを指定できる", () => {
        expect(clampHeightOffsetM(-1, -0.5, 0.5)).toBe(-0.5);
        expect(clampHeightOffsetM(1, -0.5, 0.5)).toBe(0.5);
    });
});
...
describe("isInsideDeadZone", () => {
    it("既定のヒステリシス幅は0.05m", () => {
        expect(DEFAULT_DEAD_ZONE_HYSTERESIS_M).toBe(0.05);
    });

    it("外側にいた場合、距離が半径以下になった時点でデッドゾーン内と判定する", () => {
        expect(isInsideDeadZone(0.3, false, 0.35)).toBe(true);
        expect(isInsideDeadZone(0.35, false, 0.35)).toBe(true);
        expect(isInsideDeadZone(0.36, false, 0.35)).toBe(false);
    });

    it("内側にいた場合、半径+ヒステリシスを超えるまでデッドゾーン内のまま維持する", () => {
        expect(isInsideDeadZone(0.39, true, 0.35)).toBe(true);
        expect(isInsideDeadZone(0.399, true, 0.35)).toBe(true);
        expect(isInsideDeadZone(0.41, true, 0.35)).toBe(false);
    });

    it("カスタムのヒステリシス幅を指定できる", () => {
        expect(isInsideDeadZone(0.5, true, 0.35, 0.2)).toBe(true);
        expect(isInsideDeadZone(0.56, true, 0.35, 0.2)).toBe(false);
    });

    it("非有限値のdistanceMは前フレームの判定を維持する", () => {
        expect(isInsideDeadZone(NaN, true, 0.35)).toBe(true);
        expect(isInsideDeadZone(NaN, false, 0.35)).toBe(false);
    });

    it("負のdeadZoneRadiusMは前フレームの判定を維持する", () => {
        expect(isInsideDeadZone(0.1, false, -1)).toBe(false);
        expect(isInsideDeadZone(0.1, true, -1)).toBe(true);
    });
});

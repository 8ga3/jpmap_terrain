...
describe("createDioramaTerrain の入力検証", () => {
    ...
    it("DEM取得とテクスチャ取得が並列に開始される（直列にならない）", async () => {
        // どちらのモックも意図的に解決を保留し、「両方とも呼ばれた」ことのみを検証する
        // （タイマー精度に依存する Date.now() 差分比較はCI負荷でフレークし得るため避ける）。
        // createDioramaTerrain の内部（buildMesh）は Promise.all([fetchDioramaElevations(...),
        // buildDioramaMosaicTexture(...)]) に到達するまで同期的に実行されるため、この呼び出し
        ...
        mockFetchElevations.mockImplementationOnce(
            () =>
                new Promise((resolve) => {
                    resolveElevations = resolve;
                }),
        );
        mockBuildTexture.mockImplementationOnce(
            () =>
                new Promise((resolve) => {
        ...
        const pending = createDioramaTerrain(dummyScene, baseOptions);

        // 直列実装であればこの時点で buildDioramaMosaicTexture はまだ呼ばれていないはず。
        expect(mockFetchElevations).toHaveBeenCalledTimes(1);

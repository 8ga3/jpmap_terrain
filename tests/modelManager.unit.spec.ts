...
describe("importLoaderForUrl", () => {
    test(".glb で glTF ローダーがインポートされる", async () => {
        await expect(
            importLoaderForUrl("assets/model.glb"),
        ).resolves.toBeUndefined();
    });

    test(".gltf で glTF ローダーがインポートされる", async () => {
        await expect(
            importLoaderForUrl("assets/model.gltf"),
        ).resolves.toBeUndefined();
    });

    test(".obj で OBJ ローダーがインポートされる", async () => {
        await expect(
            importLoaderForUrl("assets/model.obj"),
        ).resolves.toBeUndefined();
    });

    test(".stl で STL ローダーがインポートされる", async () => {
        await expect(
            importLoaderForUrl("assets/model.stl"),
        ).resolves.toBeUndefined();
    });
    ...
    test("クエリ文字列付き URL でも正しくロードされる", async () => {
        await expect(
            importLoaderForUrl("assets/model.obj?v=1"),
        ).resolves.toBeUndefined();
    });

    test("フラグメント付き URL でも正しくロードされる", async () => {
        await expect(
            importLoaderForUrl("assets/model.stl#section"),
        ).resolves.toBeUndefined();
    });

    test("未対応拡張子 (.fbx) は throw", async () => {
        await expect(importLoaderForUrl("assets/model.fbx")).rejects.toThrow(
            /unsupported file format/,
        );
    });

    test("拡張子なし URL は throw", async () => {
        await expect(importLoaderForUrl("assets/model")).rejects.toThrow(
            /unsupported file format/,
        );
    });
});

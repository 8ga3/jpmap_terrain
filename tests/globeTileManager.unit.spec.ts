...
vi.mock("@babylonjs/core/Materials/Textures/texture", () => {
    const TextureMock = vi.fn().mockImplementation(function (
        ...args: unknown[]
    ) {
...
const { createGlobeTileManager } = await import(
    "../src/terrain/geo/globeTileManager"
);
...
describe("createGlobeTileManager", () => {
    ...
    it("geom zoom が全 DEM 404 でも粗ズーム DEM を切り出して実標高で建築する", async () => {
        ...
        loadElevationTile.mockImplementation((...args: unknown[]) => {
            ...
            if (zoom >= 15)
                return Promise.reject(new TileFetchError("404", 404));
            return Promise.resolve(
                new Float32Array(256 * 256).fill(PARENT_ELEV),
            );
        });
        ...
        const built = capturedBuilds.find(
            (b) => b.tx === 24000 && b.ty === 12000,
        );
    ...
    it("zoom-out: 低レベル(親)タイルが描画可能になるまで高レベル(子)タイルを保持する", async () => {
        ...
        const childMeshes = [0, 1, 2, 3].map(
            (i) =>
                MeshMock.mock.results[i].value as {
                    isEnabled: () => boolean;
                    dispose: Mock;
                },
        );
        ...
        const parentMesh = MeshMock.mock.results[4].value as {
            isEnabled: () => boolean;
        };
        // 親が描画可能になる前は子をまだ保持。
        childMeshes.forEach((m) => expect(m.dispose).not.toHaveBeenCalled());
    ...
    it("zoom-in: minZoom 未満の親(粗タイル)でも子が揃うまで保持し原子スワップする", async () => {
        ...
        selectedTiles = [
            tile(100, 100, 9, 500_000),
            tile(101, 100, 9, 500_000),
            tile(100, 101, 9, 500_000),
            tile(101, 101, 9, 500_000),
        ];
    ...
    it("同一ズーム隣接の実標高タイル辺を平均化してタイル境界の段差を解消する", async () => {
        ...
        expect(
            (left as { geomElev: Float32Array }).geomElev[mid + 255],
        ).toBeCloseTo(150, 3);
        expect(
            (left as { geomElev: Float32Array }).geomElev[mid + 128],
        ).toBeCloseTo(100, 3);
        // 右タイルの左辺（col=0）は左隣 100m と平均され 150m。内部は 200m のまま。
        expect(
            (right as { geomElev: Float32Array }).geomElev[mid + 0],
        ).toBeCloseTo(150, 3);
        expect(
            (right as { geomElev: Float32Array }).geomElev[mid + 128],
        ).toBeCloseTo(200, 3);
    });
    ...
    it("同一ズーム隣接が無い実標高タイルは縫合せず原本標高で建築する", async () => {
        ...
        expect(
            (built as { geomElev: Float32Array }).geomElev[mid + 255],
        ).toBeCloseTo(100, 3);
        expect(
            (built as { geomElev: Float32Array }).geomElev[mid + 0],
        ).toBeCloseTo(100, 3);
    });
    ...
    it("日付変更線をまたぐ x=0 と x=limit-1 の同一ズーム隣接を wrap して縫合する", async () => {
        ...
        expect(
            (west as { geomElev: Float32Array }).geomElev[mid + 0],
        ).toBeCloseTo(150, 3);
        // x=1023 の右辺（col=255）は wrap 隣接 x=0 の 100m と平均され 150m。
        expect(
            (east as { geomElev: Float32Array }).geomElev[mid + 255],
        ).toBeCloseTo(150, 3);
    });
    ...
    it("取得失敗(404)の湖面タイルを 0m でなく隣接タイルの接線標高で平坦建築する", async () => {
        ...
        const centerBuilds = capturedBuilds.filter(
            (b) => b.tx === 100 && b.ty === 100,
        );
    ...
    describe("continuous モード / drainBuildQueue（連続カメラ移動時のフレーム分散）", () => {
        ...
        it("freshFrustum で視界外と判定されたジョブは、desiredKeys がまだ古いままでもビルドせず破棄する", async () => {
            ...
            mgr.drainBuildQueue({
                frustumPlanes: allCulledFrustum,
                cameraEcef,
            });
        ...
        it("freshFrustum で視界内と判定されたジョブは、従来通りビルドされる", async () => {
            ...
            mgr.sync({ ...syncParams(), continuous: true });
            mgr.drainBuildQueue({
                frustumPlanes: allVisibleFrustum,
                cameraEcef,
            });

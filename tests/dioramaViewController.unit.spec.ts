...
describe("createDioramaViewController", () => {
    it("初期状態はコンストラクタに渡した center/footprintHalfSizeM を返す", () => {
        ...
        const vc = createDioramaViewController(
            terrain,
            INITIAL_CENTER,
            INITIAL_FOOTPRINT_HALF_SIZE_M,
        );
    ...
    it("ズーム入力でsetViewが呼ばれ、footprintHalfSizeMが更新される（centerは変化しない）", async () => {
        ...
        expect(patch.footprintHalfSizeM).toBeLessThan(
            INITIAL_FOOTPRINT_HALF_SIZE_M,
        );

        await Promise.resolve();
        expect(vc.getFootprintHalfSizeM()).toBeLessThan(
            INITIAL_FOOTPRINT_HALF_SIZE_M,
        );
    });
    ...
    it("footprintHalfSizeMは既定の下限・上限でクランプされる", () => {
        ...
        const patch = setView.mock.calls[0][0];
        expect(patch.footprintHalfSizeM).toBeLessThanOrEqual(
    ...
    it("setViewが失敗した場合、centerとfootprintHalfSizeMは確定させず次回flushで再送する", async () => {
        ...
        expect(secondPatch.footprintHalfSizeM).toBe(
            firstPatch.footprintHalfSizeM,
        );
    ...
    describe("setView", () => {
        ...
        it("footprintHalfSizeMは既定の下限・上限でクランプしてから送信する", async () => {
            ...
            await vc.setView({
                footprintHalfSizeM: DEFAULT_FOOTPRINT_HALF_SIZE_MAX_M * 10,
            });

            const patch = setView.mock.calls[0][0];
            expect(patch.footprintHalfSizeM).toBe(
                DEFAULT_FOOTPRINT_HALF_SIZE_MAX_M,
            );
            expect(vc.getFootprintHalfSizeM()).toBe(
                DEFAULT_FOOTPRINT_HALF_SIZE_MAX_M,
            );
        });
        ...
        it("失敗時は状態を確定させず、呼び出し元へエラーをrejectする", async () => {
            ...
            await expect(
                vc.setView({ footprintHalfSizeM: 500 }),

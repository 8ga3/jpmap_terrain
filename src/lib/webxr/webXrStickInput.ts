/**
 * WebXR コントローラー/タッチ入力を、地図移動（パン）・拡大縮小（ズーム）・回転・高さ変更の
 * 移動量へ変換する純粋関数群（公開API）。
 *
 * @remarks
 * DOM / Babylon.js Scene / WebXR に依存しないため、シーン構築前でも単体テスト可能。
 * 実際の入力取得（XRコントローラーのthumbstick・画面タッチのGUI）とシーンへの適用は
 * 呼び出し側（各デモ実装）が担う。
 *
 * 角度規約: 本モジュール内の水平方向角度は全て `atan2(x, z)`（`z`軸=0rad、`x`軸=+π/2rad、
 * 時計回りが正方向）に統一する。Babylonの左手系Y軸回転規約（`rotation.y`が正方向で
 * +Z軸から+X軸へ回転する）と一致するよう選んでいるため、{@link computeRotationRadFromStick}
 * が加算する回転角とも整合する。
 */
 ...
 *
 * @remarks
 * 1本のスティックでパン以外の2つの操作（例: 回転とズーム）をX/Y軸に割り当てる場合、
 * 上下・左右のいずれか一方だけを操作するつもりで倒しても、物理スティックの構造上
 * わずかに斜めにずれてしまいやすい。X/Yを完全に独立してデッドゾーン処理すると、
 * 「下へ倒して操作Aをしているつもりが、わずかな左右のずれで操作Bも同時に発火してしまう」
 * という誤操作が起きる。
 *
 * 本関数は、X/Yのうち絶対値が大きい方（支配的な軸）のみ元の値を残し、もう一方を
 * 強制的に `0` にすることで、常にどちらか一方の軸のみが有効になる十字ボタン相当の
 * 入力へ変換する。速度自体は従来通りアナログ（倒し具合に比例）のまま、同時発火のみを防ぐ。
 *
 * 呼び出し側で各軸のデッドゾーン処理（{@link applyStickDeadzone}を内部で使う
 * {@link computeRotationRadFromStick}/{@link computeZoomFactorFromStick}）を行う
 * 前段として使うこと。個別ボタン等もともと排他的な入力には適用不要。
 *
...
export interface PanFromStickOptions {
    deadzone?: number;
    minViewScaleForSpeedM?: number;
}

/** パン速度計算で表示スケールが極端に小さい場合の下限[m]（操作不能なほど遅くならないようにする）。 */
export const DEFAULT_MIN_VIEW_SCALE_FOR_PAN_SPEED_M = 20;

/** 1秒間フルでスティックを倒し続けたときのパン速度（viewScaleMに対する倍率/秒）の既定値。 */
export const DEFAULT_PAN_SPEED_PER_SEC = 0.6;

/**
 * スティック入力から、1フレーム分のパン移動量[m]（東西・南北）を算出する。
 *
 * 速度は現在の表示スケール `viewScaleM`（表示中の範囲を代表する長さ。例: 表示半径や
 * カメラ高度）に比例させる（広域表示している時は速く、詳細表示している時はゆっくり動く）。
 *
 * @param viewScaleM 現在の表示スケール[m]（例: フットプリント半辺長、表示半径など）。
 * @param basePanSpeedPerSec viewScaleMに対する秒間倍率（フルスティック時、
 *   `viewScaleM * basePanSpeedPerSec` が秒速[m/s]になる）。
...
export const computePanMetersFromStick = (
    ...
    const minViewScaleM = options.minViewScaleForSpeedM ?? DEFAULT_MIN_VIEW_SCALE_FOR_PAN_SPEED_M;
    const x = applyStickDeadzone(axes.x, deadzone);
    const y = applyStickDeadzone(axes.y, deadzone);
    if (x === 0 && y === 0) return { eastM: 0, northM: 0 };
    const effectiveViewScaleM = Number.isFinite(viewScaleM) ? Math.max(viewScaleM, minViewScaleM) : minViewScaleM;
    const speed = basePanSpeedPerSec * effectiveViewScaleM;
    return {
 ...
 * 前方向・右方向の水平単位ベクトルへ投影し、{@link computePanMetersFromStick}
 ...
 * 例: カメラの向き基準（キーボード操作）、ユーザー位置↔注視対象の向き基準（AR中）等。
 *
...
export const computePanAxesFromDirectionalInput = (
    ...
    // `computePanMetersFromStick` の規約（y軸は前方向が負値）に合わせて
 ...
 * 分解する。「ユーザー位置から注視対象中心への向き」等の算出に使う。
 *
...
/**
 * ユーザー（実機カメラ）が注視対象に重なるように立っている（デッドゾーン内）かどうかを、
 * ヒステリシス付きで判定する。
 *
 * @remarks
 * 注視対象のすぐ近く・真上にユーザーが立つと、「ユーザーから対象中心への向き」
 * （{@link computeHorizontalDisplacement}）が不安定になる（距離が0に近づくほど
 * わずかな立ち位置のずれで向きが大きく変わる）。この状態でパン方向を計算しても
 * 実用的な結果にならないため、デッドゾーン内ではパン入力自体を無効化する用途を想定する。
 *
 * @param distanceM ユーザーから対象中心までの水平距離[m]（{@link computeHorizontalDisplacement}の`distanceM`）。
 * @param wasInsideDeadZone 前フレームの判定結果。
 * @param deadZoneRadiusM デッドゾーンの半径[m]（通常は対象の表示半径）。
...
/** ズームの秒間倍率既定値（1秒間フルで倒すと表示スケールが概ね1/2倍/2倍になる）。 */
export const DEFAULT_ZOOM_RATE_PER_SEC = 2;

/** 表示スケールの下限・上限既定値[m]。 */
export const DEFAULT_VIEW_SCALE_MIN_M = 100;
export const DEFAULT_VIEW_SCALE_MAX_M = 2_000_000;

/**
 * 右スティックY軸（またはGUIズームボタン相当の軸値）から、1フレーム分の
 * 表示スケールへの乗算係数を算出する（乗算方式）。
 *
 * 前に倒す（y<0）ほど表示スケールが縮む（ズームイン、より詳細な狭い範囲を表示）。
 *
 * @param zoomRatePerSecond 1秒間フルで倒し続けたときの倍率（>1 を想定）。
 * @returns 表示スケールに乗算する係数（入力なしや不正な dt なら 1 = 変化なし）。
 */
export const computeZoomFactorFromStick = (
    axisY: number,
    dtSeconds: number,
    zoomRatePerSecond: number = DEFAULT_ZOOM_RATE_PER_SEC,
 ...
 */
export const clampViewScaleM = (
    scaleM: number,
    minM: number = DEFAULT_VIEW_SCALE_MIN_M,
    maxM: number = DEFAULT_VIEW_SCALE_MAX_M,
): number => {
    const safe = Number.isNaN(scaleM) ? minM : scaleM;
    return Math.min(maxM, Math.max(minM, safe));
};

/** 回転操作の秒間最大角速度[rad/s]（フルスティック時）の既定値。 */
export const DEFAULT_ROTATION_SPEED_RAD_PER_SEC = Math.PI / 2; // 90°/秒

/**
 * 右スティックX軸（またはキーボード等価入力の軸値）から、1フレーム分の回転角[rad]を
 ...
 */
export const computeRotationRadFromStick = (
    axisX: number,
    dtSeconds: number,
 ...
 */
export const computeHeightMetersFromTriggers = (
...
/**
 * 高さオフセットを [minM, maxM] へクランプする。
 * `NaN` は 0（オフセット無し）へフォールバックしてからクランプする
 * （{@link clampViewScaleM} と同じ方針）。
 */
export const clampHeightOffsetM = (
    offsetM: number,

 ...
 *
 * 操作割り当て（#539〜#542で確定した一覧、以降のサブタスクもこれに従う）:
 * - 左スティック / GUI仮想ジョイスティック: 地図中心の東西・南北移動（パン）
 * - 右スティックY（前後） / GUIズームボタン: フットプリント半径のズーム
 *   （前方向・GUIの「+」= ズームイン/縮小、後方向・GUIの「-」= ズームアウト/拡大）
 * - 右スティックX（左右）: 箱庭の回転（#540で実装）
 * - トリガー（左右）: 箱庭の設置高さ変更（#540で実装）
 * - グリップ + 左スティック（モディファイア）: 太陽の方位角・高度（#541で実装）
 * - A/Xボタン: 地図タイル種別切替（#542で実装）
 * - B/Yボタン: トップ（初期center/footprintRadius）復帰（#542で実装）
 */

 ...
 * - Worker / OffscreenCanvas / createImageBitmap が利用不能な環境では
 *   呼び出し側に `null` を返し、従来の同期パスへフォールバックさせる
 * - `{ type: "module" }` は指定せず classic worker として生成する
 *   （既存 elevationWorker.ts と同じパターンに揃え、webpack の余計な
 *   束ね込みで再帰的に Worker が立ち上がるのを避ける）
 */
import type {
    DecodeRequest,
    DecodeSuccess,
    ...
    for (let i = 0; i < poolSize; i++) {
        try {
            ...
            w.onerror = (ev: ErrorEvent) => {
                console.warn("[jpmap-terrain] elevation decode worker error:", ev.message);
                // 該当 worker の pending タスクを巻き戻すのは困難なので、
                // 既存 pending はそのまま残し、応答待ちが timeout するのを許容する。
                // （pool 自体は他 worker と共に継続）
            };
            workers.push(w);
        } catch (err) {
...
export const getElevationDecodePool = (): ElevationDecodePool | null => {
    ...
    try {
        ...
        console.warn("[jpmap-terrain] elevation decode pool init failed:", err);
        cacheFailed = true;
        return null;
    }
};

...
const buildMesh = async (
    ...
    // 地形面自体は問題なく描画されているが、側面壁が深度比較上「手前」として
    // 扱われ、地形面を覆い隠していたことが判明した。
    //
    // 当初 `renderingGroupId` を分けるだけで地形面を側面壁より後段に描画させたが、
    // Babylon はレンダリンググループが切り替わるたびに既定で深度バッファを
    // クリアする（`RenderingManager` の `_autoClearDepthStencil` は全グループが
    // 既定 `autoClear: true`）ため、後段グループのメッシュは常に「深度バッファが
    // まっさら」な状態から描画され、実際の前後関係と無関係に必ず前段グループを
    // 覆ってしまう（Androidスマホ実機で「地図は出たが壁が消えた」症状として発覚。
    // Quest 3では逆に側面壁が地形面を覆っていたが、これも同じグループ内での
    // 描画順依存の症状であり、根本原因は同じ）。
    // `setRenderingAutoClearDepthStencil` で後段グループの自動クリアを無効化し、
    // 前段グループ（側面壁）が書き込んだ深度値をそのまま引き継いで正しく
    // 深度テストされるようにする。これにより、描画順を分離しつつ
    // （どちらのメッシュが先に処理されても安定させる）、実際の3D上の前後関係で
    // 正しく隠面消去されるようになる。

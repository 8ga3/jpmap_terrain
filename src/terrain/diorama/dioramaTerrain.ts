...
const buildMesh = async (
    ...
    // 不具合の原因を段階的に切り分けた結果、テクスチャの生成方式・サイズは無関係で、
    // 地形メッシュ自体の描画（バックフェースカリング）が原因と判明した。
    // デスクトップのプレビューカメラは常に急な俯角（見下ろし角、既定で鉛直から60°）
    // で地形を見るため、法線と視線方向の内積は明確に正（表向き）だが、AR中は
    // 実際のヘッドセット/端末の高さ（既定の卓上配置 {@link AR_TABLE_HEIGHT_M} と
    // 近い高さになりがち）から見るため、視線がほぼ水平・地形面をかすめる角度になり
    // やすい。この「かすめる角度」では法線と視線の内積が0付近になり、既定の
    // `backFaceCulling=true` の表裏判定が不安定になって地形面全体がカリングされて
    // しまう（側面壁 `skirtMaterial` は元々 `backFaceCulling=false` にしていたため
    // 影響を受けず、常に見えていた）。地形面も側面壁と同様にカリングを無効化する。

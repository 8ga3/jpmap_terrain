...
server {
    ...
    location / {
        ...
        rewrite ^/(viewer|timelapse|polygon|distance|circle|plan|gpx|model|avatar|avatar-controller|boids|flight|artillery|geospatial|zoomloop|roiorbit|diorama)(?:/.*)?$ /$1.html last;
...
# デモ識別子付きパス（/viewer, /viewer/@lat,lon,...）を実体HTMLへ書き換える。
# demoAtPathRewrites（vite.rewrites.ts）と同じデモ名一覧を維持すること。
RewriteRule ^(viewer|timelapse|polygon|distance|circle|plan|gpx|model|avatar|avatar-controller|boids|flight|artillery|geospatial|zoomloop|roiorbit|diorama)(/.*)?$ /$1.html [L]
```
...
- 既定中心: 富士山・富士宮口五合目付近の山腹（35.3436, 138.7203。単調な斜面が見える地点）
- 既定の実世界フットプリント半辺長: 800m（操作可能範囲: 100m〜2,000,000m）
- 既定の卓上表示半径: 0.35m（デスクトップのカメラズーム範囲は 0.42m〜5.25m。中心から最も遠い点＝正方形の対角線の先端＝四隅までの距離がこの半径になるようスケールする）
- **外形は正方形（円形ではない）**: 当初は「丸く（円形に）クリップされた箱庭」を要件としていたが、Meta Quest 3 実機での WebXR パフォーマンス（放射状グリッドの頂点密集によるシワ状アーティファクト・地形再構築の重さ）を優先し、円形マスク処理（シェーダー discard / ステンシル / ジオメトリクリップ）は実装せず、正方形グリッドの外周そのままの見た目に変更する合意を得ている
- 外周から一定深さ下へ土色の側面壁・底面を張り、実物のジオラマ模型のような外観にする（側面壁は頂点カラーで上端が明るく下端が暗いグラデーション）
- タイル種別は3種を巡回切替: `std`（標準地図）→ `photo`（写真）→ `wireframe`（ラスタタイル（地図画像）の取得・モザイク合成をスキップし、地形形状のみを単色ワイヤーフレーム表示。標高（DEM）取得は他の2種と同様に行う）
- デスクトップのカメラは回転＋ズームのみに対応（pan は無効化。正方形の箱庭は pan するとフレームアウトし戻す手段がないため）
- WebXR (`immersive-ar`) セッション統合により、パススルー表示で箱庭の周りを歩いて見られる。AR突入時はテーブル高さ相当（0.7m）にユーザー正面基準で配置する（床検出によるテーブル高さ推定は対象外）
- **太陽位置移動によるライティング操作は実装していない**。操作が複雑になり機能的にも過剰と判断し開発をキャンセルしたため、太陽光は固定の `DirectionalLight` のまま操作対象にしていない
- AR非対応環境・AR突入前の通常表示でも、常時表示のタッチHUD（仮想ジョイスティック＋ボタン）で全操作が可能（グレーアウト等のアクセス制限はしない。箱庭の見た目自体に単体デモとしての価値があるため）
...
| 箱庭の設置高さ変更 | 左右トリガー | 「▲」（上げる）/「▼」（下げる）ボタン | `Z`（下げる） / `X`（上げる） |
| タイル種別切替（std→photo→wireframeの巡回、単発トリガー） | A/Xボタン | タイル切替ボタン（単発タップ） | `T`（単発トリガー、キーリピートは無視） |
| AR終了・通常表示へ復帰（単発トリガー） | B/Yボタン | 「⌂」ボタン（単発タップ。AR中でなければ意味を持たないため、AR非対応環境・AR突入前の常時HUDでは無効化＝グレーアウト） | 割り当てなし（デスクトップキーボードはAR非対応/AR突入前のPC単体動作確認用途のため） |

- 右スティックは十字ボタン相当の排他動作にしている（X＝回転・Y＝ズームのいずれか支配的な軸のみ有効。物理スティックの構造上わずかに斜めへずれて同時発火するのを防ぐ。GUIのボタンはもともと個別で排他的なため対象外）
- 「AR終了・通常表示へ復帰」は、「トップ画面復帰」要件を、ARモードを終了し通常表示へ戻る操作として実装したもの（デモポータル `/` への遷移ではない）。画面左上の「← デモ一覧へ」リンク（`public/diorama.html`）が別途ポータルへの復帰導線を提供する
- パン方向はキーボード・タッチHUDともにカメラの現在の水平向き基準、AR中の物理コントローラーは「ユーザー（実機カメラ）から見て箱庭中心の奥」＋「箱庭自体の回転」基準（詳細は `dioramaArControls.ts` 冒頭コメント参照）

**URL:** `?engine=webgpu|webgl|webgl2` のみ対応（既定は他デモと異なり `webgl2`。理由は AR中の `requiredFeatures` へ `"webgpu"` が自動追加されると Meta Quest Browser 等が拒否するため）。他デモと異なり `/@lat,lon[,...]` によるカメラ初期位置指定・`?mapType=` は未対応（既定中心・既定タイル種別で起動する固定仕様）。

**実機検証:** WebXR のセキュアコンテキスト要件（HTTPS/localhost必須）のため、Meta Quest 3 等での実機検証には Cloudflare トンネル（`docker/compose.webxr-tunnel.yaml`）を使う（`docker/README.md` 参照）。

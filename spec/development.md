...
Netlifyへのデプロイは `.github/workflows/deploy.yml` により、**タグのpush**をトリガーに実行される（`push.tags: ['**']`、パターン指定なし。`'*'` では `/` を含むタグにマッチしないため `'**'` を使用している）。
...
- リリース手順:
  ...
  3. `main` の最新コミットにタグを付ける（例: `git tag v0.3.0`）
  4. タグをpushする（`git push origin v0.3.0`）
